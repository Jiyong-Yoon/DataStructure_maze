...
from DS_class import CQueue

m, MAZE_SIZE = load_maze("map_file_2.txt")
maze = [list(row) for row in m]

# 시작점과 도착점 좌표 찾기
start_pos, end_pos ,= find_points_queue(maze)
=======
...
>>>>>>> 63ddf4d3c9e677b35329c5e19e94f850eaa85981
        ...
        #이동 가능한 방향 확인 후 스택에 푸시
<<<<<<< HEAD
        if wall(x, y-1, maze, MAZE_SIZE): d.enqueue((x, y-1))
        if wall(x, y+1, maze, MAZE_SIZE): d.enqueue((x, y+1))
        if wall(x-1, y, maze, MAZE_SIZE): d.enqueue((x-1, y))
        if wall(x+1, y, maze, MAZE_SIZE): d.enqueue((x+1, y))
