...
def load_maze(filename):
    with open(filename, 'r') as file:
        ...
        exec_locals = {} #로컬 딕셔너리 생성
        exec(content, {}, exec_locals) # exec() 함수로 코드 실행
        
        # m과 MAZE_SIZE 값 추출
        return exec_locals['m'], exec_locals['MAZE_SIZE']

m, MAZE_SIZE = load_maze("map_file_1.txt")
maze = [list(row) for row in m]

# 시작점과 도착점 좌표 찾기
start_pos, end_pos = find_points_stack(maze)
...
<<<<<<< HEAD
        ...
        if isValidPos(x+1, y, maze, MAZE_SIZE): s.push((x+1, y)) 
        if isValidPos(x-1, y, maze, MAZE_SIZE): s.push((x-1, y))  
        if isValidPos(x, y-1, maze, MAZE_SIZE): s.push((x, y-1))
        if isValidPos(x, y+1, maze, MAZE_SIZE): s.push((x, y+1))  
=======
...
<<<<<<< HEAD
        ...
        if wall(x, y-1, maze, MAZE_SIZE): d.push((x, y-1))
        if wall(x, y+1, maze, MAZE_SIZE): d.push((x, y+1))
        if wall(x-1, y, maze, MAZE_SIZE): d.push((x-1, y))
        if wall(x+1, y, maze, MAZE_SIZE): d.push((x+1, y))
